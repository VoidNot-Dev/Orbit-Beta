...
export class SearchOrchestrator {
    ...
    async doSearches(
        ...
        if (useParallel) {
            return await this.doParallelSearches(data, missingSearchPoints, mobileSession, account, accountEmail)
        } else {
            return await this.doSequentialSearches(data, missingSearchPoints, mobileSession, account, accountEmail)
        }
    }
    ...
    private async doParallelSearches(
        ...
        try {
            ...
            if (shouldDoMobile) {
                ...
                promises.push(
                    this.doMobileSearch(data, missingSearchPoints, mobileSession, account, accountEmail).then(
                        points => {
            ...
            if (shouldDoDesktop && desktopSession) {
                ...
                promises.push(
                    this.doDesktopSearch(data, missingSearchPoints, desktopSession, account, accountEmail).then(
                        points => {
    ...
    private async doSequentialSearches(
        ...
        if (shouldDoMobile) {
            ...
            mobilePoints = await this.doMobileSearch(data, missingSearchPoints, mobileSession, account, accountEmail)
        ...
        if (shouldDoDesktop) {
            ...
            desktopPoints = await this.doDesktopSearchSequential(data, missingSearchPoints, account, accountEmail)
